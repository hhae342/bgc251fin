  ...
  for (let idY = 0; idY < height; idY += 15) {
    for (let idX = 0; idX < width; idX += 15) {
      particles.push({
        ...
        vx: 0,
        vy: 0,
        colour: color(random(170, 255), 100, 100),
      });
    }
  }
}
...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    let Vx = map(
      noise(particle.x * 0.00001, particle.y * 0.005, frameCount * 0.001),
      0,
      1,
      minSpeed,
      maxSpeed
    ...
    let Vy = map(
      noise(particle.y * 0.00001, particle.x * 0.005, frameCount * 0.01 + 1000),
      0,
      1,
      minSpeed,
      maxSpeed
